...
process.stdin.on('end', () => {
  ...
  const additionalContext = [
    ...
    '   first steps: `git fetch origin && git worktree add -b <branch> <path>',
    '   origin/main` — cut from origin/main, never from whatever HEAD the checkout is',
    '   parked on. Then `gh pr list --state all --search "<keyword>"`; already landed',
    '   or in flight means STOP and report. Every path, line number, count and quote',
    '   you write is a SNAPSHOT — say so, and tell it to re-read before trusting.',
    '',
    ...
    '   e2e green; open the PR THEN one round of scripts/dev/grok-review.sh and answer',
    '   findings in a PR comment; any attached frame comes from `npm run dev -- --mode',
    '   walk` and never plain dev, because the remote is PUBLIC and real profiles',
    '   publish the household\'s names and loads; DO NOT MERGE — the owner decides.',
  ].join('\n')
