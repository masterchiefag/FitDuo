...
- Generator emits per-`WorkItem` `workSeconds` (max across participants of `setSeconds`). **A 15s `changeover` is a player *phase*, not a plan item** — inserted between consecutive work items **within the same round**, when the next item's `exerciseId` differs — so `plannedSets`, `setIndex`, and feedback never walk timed gaps. It must **not** fire on the last-item→next-round edge: rest already sits there, and stacking a changeover onto every rest adds ~2 minutes to a full day that `fitToBudget` never sees — worse on a short one. `estimatePlanSeconds` must **add** `(items.length − 1) × 15 × rounds` for work blocks — that term is **not in the function today**, and a player-only changeover leaves `fitToBudget` blind to ~3¼ min on a typical full day (3 two-item supersets + a 3-item circuit): a 20-minute request would play ~22–23, and a day the fitter pinned at 60:00 would run past the band. Bumping `generatorVersion` to 2 is not a substitute for the estimator change.
- Reducer: `work` becomes a **timed** phase (`endsAt`). Flow on expiry: auto-log sets → changeover (if the exercise changes) → next work; end of round → rest; end of block → block transition. `SET_DONE` = "finish early"; new `EXTEND_WORK` (+15s); new `ADJUST` event stores per-person override targets **in reducer state**, scoped to the **current work item only** and cleared on leaving it (including Skip) — an override must never leak onto later exercises. **The auto-log on work expiry reads the pending override**, not just `SET_DONE`: expiry is the follow-along path and finishing early is the exception, so an override honoured only on the rare path is one that silently vanishes in normal use.
...
- **The block gate — one tap, and it's what moves the weights.** The gate **replaces the block's final `work → block_transition` edge**: a **hold** (timer at zero, nothing auto-advances) showing one row per exercise per person with **easy / hard** chips and a single large **Continue**. It is *not* a rest expiry — rest sits between rounds and the last round has none, so a hold built on rest would fire after round 1 with two rounds still to go and never at the end of the block. Semantics:
...
- **Assumed sets — precise semantics** (the earlier wording froze progression permanently, since hands-off follow-along marks *every* set assumed):
  - **Unadjusted** auto-logged sets carry `assumed: true` and **do count as "target hit" for progression**, because that is exactly the follow-along contract — the app called the reps, you did them, like a class.
  - **`assumed` is true only when the log used the *prescribed* target.** An auto-log that consumes a pending `ADJUST` override is an *adjusted* set — `assumed: false`, actuals from the override — because after R1 the natural way to correct a set is to tap adjust and let the beep log it. Marking that path assumed would make a downward correction read as a target hit (they say 8, next session prescribes 11) and would drop the PR off an upward one.
  - **Every `LOG_SET` path consumes the pending override and then clears it — expiry *and* `SET_DONE`.** Once overrides live in reducer state they no longer ride on the Done click (`PlayerScreen` stuffs them onto `SET_DONE.overrides` today), so a finish-early tap after Adjust would otherwise log the prescribed target.
...
- **Short session — ✅ mechanism shipped for mobility 2026-08-14; reuse it for strength.** `targetSeconds` is now a first-class generator input (mobility offers 5/10/20/30 min and fills the budget). R1 applies the same input to the strength generator instead of inventing a second mechanism — but applying it is more than forwarding the argument: `fitToBudget` fits against a hard `[DURATION_MIN_S, DURATION_MAX_S]` band that a type comment and a property test both restate, so **the band has to become a function of the input** or a 20-minute request still emits a 50-minute plan. `sessionXp` likewise pays 50 base + 25 full-clear off `mode`, so a short session needs **its own `mode` value and XP rule** rather than billing as a full one. Full streak credit, proportionally less XP (it's honest, and the streak — not XP — is what protects motivation). Offered on Today as a secondary button, and as a mid-session **"cut it short"** which is an *explicit completion transition*, not an abandon: it truncates the plan at the current block boundary, **rewrites the session's planned-set count to what was actually programmed through that block**, and completes. (Without the rewrite, the Architecture's "completed = ≥60% of planned sets" rule would mark exactly the sessions R4 exists to rescue as abandoned.) **Do not implement that 60% rule in order to give the rewrite something to satisfy** — nothing implements it today, completion is the `ABANDON` flag, and adding it here would be building the problem the rewrite solves.
...
1. **R1** — follow-along player (the core feel change; rewrites the state machine, so it goes first and alone). Fold in R4's **short session** (targetSeconds input + 'cut it short' completion transition) since both touch session shape. **One PR.** It is a wide diff — generator, reducer, player screen, fuzz, e2e — and splitting it was considered and rejected: every seam produces an intermediate app worse than both sides of it. Timed work without the block gate collects no feedback, so progression freezes or ratchets with the superseded per-exercise taps still nailed on; the gate without `ADJUST` is the hands-free session with no way to say "I did 8, not 10", which is the ratchet with the correction path deliberately withheld. Three review tails would buy three chances to merge a worse app than the one that exists today.
   **Done when the end-to-end drive passes in a real browser: press Start, touch nothing but Continue, reach the celebration** — then the same session corrected mid-set (the log carries the correction, the next exercise does not inherit it), and a 20-minute session run to the end.
