...
1. **R1** — follow-along player (the core feel change; rewrites the state machine, so it goes first and alone), **landed as three PRs**. R1 in one diff is the generator, the reducer, a 756-line player screen, new fuzz and new e2e; this repo's own review history says a diff that size does not converge (PR #1 closed eleven findings, PR #3 took eight rounds, both far smaller). But the split is by **capability, never by layer or by increment**: each PR is a thing someone can start in the browser and finish, and is verified that way before it merges. A PR landing a lone property test, or a reducer without the screen that renders it, has nothing to drive end to end — and "a green suite is not a working app" is the standing bar.
   1. **R1a — the hands-free session, whole.** `workSeconds` on `WorkItem`; `work` gains `endsAt`; the 15s changeover phase; the late-timer rule extended so fast-forward never crosses a work phase; `assumed: true` on auto-logged sets; `SET_DONE` demoted to finish-early; the countdown ring and pacing hint; and **the block gate**, which replaces each strength block's final `work → block_transition` edge — per-exercise, per-person easy/hard chips, one large Continue, unrated ⇒ `'right'`, unanswered for 5 minutes pauses the session. It is *not* hung off a rest expiry: rest sits between rounds and the last round has none, so a hold built on rest would fire after round 1 with two rounds still to go and never at the end of the block. Also lands `weightSnap` + `repClamp` as a property test — no rule can emit a weight that person does not own or reps outside the exercise's range — because this is the PR where targets start being prescribed with nobody confirming them.
      **End to end: press Start, touch nothing but Continue, reach the celebration.** The gate does not separate from timed work: without it nothing collects feedback, so progression freezes or ratchets and what ships is half a feature with the other half's UI still nailed on.
   2. **R1b — mid-set corrections.** `EXTEND_WORK` (+15s) and `ADJUST` (per-person overrides scoped to the current work item, cleared on leaving it, Skip included). **The auto-log on work expiry must read the pending override**, not just `SET_DONE` — expiry is the follow-along path and finishing early is the exception, so an override honoured only on the rare path is one that silently vanishes in the normal case. End to end: run a session, correct a target mid-set, confirm the log carries the correction and the next exercise does not inherit it.
   3. **R1c — short strength session (R4's half).** `targetSeconds` as a strength-generator input — more than forwarding the argument mobility already takes: `fitToBudget` fits against a hard `[DURATION_MIN_S, DURATION_MAX_S]` band that a type comment and a property test both restate, so the band has to become a function of the input or a 20-minute request still emits a 50-minute plan. `sessionXp` pays 50 base + 25 full-clear off `mode`, so a short session needs its own `mode` value and XP rule rather than billing as a full one. Plus "cut it short" as an explicit completion transition at the block boundary — and **not** by implementing Architecture's "≥60% of planned sets" rule so the planned-set rewrite has something to satisfy: nothing implements that rule today, completion is the `ABANDON` flag, and adding it would be building the problem the rewrite solves. End to end: start a 20-minute session and finish it; start a full one and cut it short at a block boundary.
