...
4. **Then one round of `scripts/dev/grok-review.sh diff` — mandatory, every PR. Grok posts its own review to the PR.** The PR has to exist first or the review has nowhere to land and survives only as your summary of it, which is not a second opinion, it is you again. Read `$OUT` afterwards — it is the full transcript, and the comment Grok posted is the review it composed from it. If no comment landed (out of turns, or no PR open), write the findings into the PR yourself — there is no command that uploads the transcript. Take what's real, decline the rest *with the reason in a PR comment*. **This is where the tail ends — the terminal state is "stop, the PR is open".** One round, not a loop: reviewer silence is never the goal, a working app is. Verification and review catch different things — the browser catches behaviour nobody modelled, review catches a number whose only witness is the code that computed it (2026-08-16, docs/DECISIONS.md).
