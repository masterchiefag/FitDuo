...
4. **Open the PR**, then run the tail against the final sha:
   - `scripts/dev/grok-review.sh diff main..HEAD` → fix → re-run it against the final sha → read it, then `scripts/dev/grok-review.sh post` → `scripts/dev/record-step.sh grok`. `post` refuses unless the review it holds was run against `HEAD`, so a fix commit means another review, not another post.
